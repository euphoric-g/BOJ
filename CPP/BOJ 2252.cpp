...
int main() {
    ...
    vint connected[n+1];
    vint count(n+1, 0);
    bool ranged[n+1] = {false, };
    auto compare = GET_N_PII(m);
    REP(i, 0, m) {
        connected[compare[i].first].push_back(compare[i].second);
        ++count[compare[i].second];
    }
    queue<int> zeros;
    REP(i, 1, n+1) if(count[i] == 0) zeros.push(i);
    vint sorted;
    while(!zeros.empty()) {
        int i = zeros.front(); zeros.pop();
        if(count[i] == 0 && !ranged[i]) {
            sorted.push_back(i);
            ranged[i] = true;
            FOREACH(it, connected[i]) {
                if(!ranged[*it]) {
                    if(--count[*it] == 0) {
                        zeros.push(*it);
                    }
