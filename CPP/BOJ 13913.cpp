...
int main() {
    ...
    while(!q.empty()) {
        ...
        while(sz--) {
            ...
            if(!visited[get.first-1]) {
                visited[get.first-1] = 1;
                get.second.push_back(get.first-1);
                q.push({get.first-1, get.second});
                get.second.pop_back();
            }
            if(!visited[get.first+1] && get.first+1 <= 100000) {
                visited[get.first+1] = 1;
                get.second.push_back(get.first+1);
                q.push({get.first+1, get.second});
                get.second.pop_back();
            }
            if(!visited[get.first*2] && get.first*2 <= 100000) {
                visited[get.first*2] = 1;
                get.second.push_back(get.first*2);
                q.push({get.first*2, get.second});
                get.second.pop_back();
            }
        }
    }
