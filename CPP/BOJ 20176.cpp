...
int main() {
    FASTIO;
    int a, b, c;
    vint va(2*RANGE+1, 0), vb(2*RANGE+1, 0), vc(4*RANGE+2, 0);
    cin >> a;
    REP(i, 0, a) {int num; cin >> num; ++va[num+RANGE];}
    cin >> b;
    REP(i, 0, b) {int num; cin >> num; ++vb[num+RANGE];}
    cin >> c;
    REP(i, 0, c) {int num; cin >> num; ++vc[num+RANGE];}
    ll cnt = 0;
    auto conv = FFT_multiply(va, vc);
    REP(i, 0, 2*RANGE+1) {
        cnt += vb[i] * conv[2*i];
    }
    cout << cnt << '\n';
