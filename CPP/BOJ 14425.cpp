...
class Trie {
    ...
    */
    ...
    Trie() { is_root = true; }
    Trie(char c) { ch = c; }
    bool exist(char to_find) { return next_bit & (1 << (to_find-'a')); }
    Trie *find(char to_find) {
        if(next_bit == 0) next = vector<Trie>(26);
        if(!exist(to_find)) { next[to_find-'a'] = Trie(to_find); next_bit |= (1 << (to_find-'a')); }
        return &next[to_find-'a'];
    }
    ...
    bool search(string &word) {
        ...
        while(cursor < word.size()) {
            ...
            cur = &cur->next[word[cursor++]-'a'];
        }
        return cur->finish;
    }
    void explore(string s) {
        if(finish) cout << s << '\n';
        REP(i, 0, 26) if(exist(i+'a')) next[i].explore(s + (char)(i + 'a'));
    }
};
// Data Structure : Trie
...
int main() {
    ...
    while(m--) {
        cin >> word;
        if(dict.search(word)) ++n;
    }
    cout << ++n << '\n';
    /*
    // 정렬 + 이분탐색 버전
    auto dict = GET_N_STR(n);
    int cnt = 0;
    SORT(dict);
    while(m--) {
        string word;
        cin >> word;
        if(binary_search(ALL(dict), word)) ++cnt;
    }
    cout << cnt << '\n';
    */
}