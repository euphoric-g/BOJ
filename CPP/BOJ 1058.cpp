...
int main() {
    ...
    auto dist = INIT_VVINT(n, n, INF);
    REP(i, 0, n) {
        REP(j, 0, n) {
            if(i == j) dist[i][j] = 0;
            else if(vec[i][j] == 'Y') dist[i][j] = 1;
        }
    }
    REP(i, 0, n) {
        REP(j, 0, n) {
            REP(k, 0, n) {
                // i -> k 로 가는데 j를 거쳐가는 경우
                if(i == j || j == k || i == k) continue;
                dist[i][k] = min(dist[i][j] + dist[j][k], dist[i][k]);
            }
        }
    }
    int max_two_friend = 0;
    REP(i, 0, n) {
        int cnt = 0;
        REP(j, 0, n) {
            if(i != j && dist[i][j] < 3) ++cnt;
        }
        max_two_friend = max(max_two_friend, cnt);
    }
    cout << max_two_friend << '\n';
}