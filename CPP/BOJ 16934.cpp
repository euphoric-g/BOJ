...
class Trie {
    ...
    */
    public:
    bool is_root = false, finish = false, prefix = false;
    char ch = 'R';
    int finish_cnt = 0;
    map<char, Trie> next;
    Trie() { is_root = true; }
    ...
    string get_prefix(string &word) {
        ...
        Trie *cur = this;
        while(cursor < word.size()) {
            if(!cur->is_root && !cur->prefix) {
                // 별칭을 찾으면 그 이후의 경로에 대해 별칭으로 사용할 수 없도록 함
                string ret = word.substr(0, cursor);
                cur->prefix = true;
                while(cursor < word.size()) {
        ...
        cur->prefix = true;
        return word + (cur->finish_cnt > 1 ? to_string(cur->finish_cnt) : "");
    }
