...
vint FFT_multiply(vint a, vint b) {
    ...
    vint res(n, 0);
    REP(i, 0, n) {res[i] = int(fa[i].real() + (fa[i].real() > 0 ? 0.5 : -0.5)); if(res[i]) res[i] = 1; }
    return res;
}
...
int main() {
    ...
    cin >> n >> k;
    vint va(1024, 0);
    REP(i, 0, n) { int t; cin >> t; va[t] = 1; }
    int ind = k-1;
    vint mul = va;
    while(ind) {
        if(ind & 1) mul = FFT_multiply(mul, va);
        ind >>= 1;
        va = FFT_multiply(va, va);
    }
    REP(i, 0, SIZE(mul)) if(mul[i] != 0) cout << i << ' '; ENDL;
}