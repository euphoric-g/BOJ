...
int find_root(int v) {
    if(uf[v] == 0) return v;
    return uf[v] = find_root(uf[v]);
}

void merge(int v, int w) {
    int vr = find_root(v), wr = find_root(w);
    uf[vr] = wr;
}
...
int main() {
    ...
    sort(ALL(vec), [](const piii &a, const piii &b) -> bool {
        return a.second < b.second;
    });
    REP(i, 0, SIZE(vec)) {
        if(find_root(vec[i].first.first) == find_root(vec[i].first.second)) continue;
        merge(vec[i].first.first, vec[i].first.second);
        weight_sum += vec[i].second;
        expensive = max(expensive, vec[i].second);
    }
    cout << weight_sum - expensive << '\n';
}