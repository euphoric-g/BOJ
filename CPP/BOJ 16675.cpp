...
string judge(int a, int b, int c, int d) {
    if(a == 0 && b == 0) return "MS\n";
    if(c == 0 && d == 0) return "MS\n";
    if(a == 1 && c == 1) return "TK\n";
    if(b == 1 && d == 1) return "TK\n";
    return "?\n";
}
...
int main() {
    ...
    s += ml, s += mr, s += tl, s += tr;
    bool can_win = false;
    int v1 = compare(ml, tl), v2 = compare(ml, tr), v3 = compare(mr, tl), v4 = compare(mr, tr);
    cout << judge(v1, v2, v3, v4);
}