...
class Trie {
    ...
    */
    ...
    vpis search_Aho_Corasick(string text) {
        ...
        REP(i, 0, n) {
            ...
            if(cur->finish) ret.push_back({i, cur->pref});
        }
        return ret;
    }
};
...
int main() {
    ...
    int l, c, w;
    cin >> l >> c >> w;
    auto poscheck = [&l, &c](pii &a) -> bool { return a.first >= 0 && a.first < l && a.second >= 0 && a.second < c; };
    auto board = GET_N_STR(l);
    unordered_map<string, pair<pii, int>> words_found;
    Trie dict = Trie();
    auto words = GET_N_STR(w);
    REP(i, 0, w) {
        dict.insert(words[i]);
    }
    dict.init_Aho_Corasick();
    REP(i, 0, l) {
        REP(j, 0, 8) {
            string s = "";
            pii cur = {i, 0};
            while(poscheck(cur)) {
                s += board[cur.first][cur.second];
                cur = {cur.first + dx[j], cur.second + dy[j]};
            }
            auto found = dict.search_Aho_Corasick(s);
            for(const auto &result : found) {
                // {i, 0}에서 j방향으로 result.first-result.second.size()+1칸 이동한 위치
                int dist = result.first - result.second.size() + 1;
                pii loc = {i + dist * dx[j], dist * dy[j]};
                if(words_found.find(result.second) == words_found.end()) {
                    words_found[result.second] = {loc, j};
                } else {
                    if(words_found[result.second].first > loc) words_found[result.second] = {loc, j};
                    else if(words_found[result.second].first == loc && words_found[result.second].second > j) words_found[result.second] = {loc, j};
                }
            }
            cur = {i, c-1};
            s = "";
            ...
            found = dict.search_Aho_Corasick(s);
            for(const auto &result : found) {
                // {i, c-1}에서 j방향으로 result.first-result.second.size()+1칸 이동한 위치
                int dist = result.first - result.second.size() + 1;
                pii loc = {i + dist * dx[j], c-1 + dist * dy[j]};
                if(words_found.find(result.second) == words_found.end()) {
                    words_found[result.second] = {loc, j};
                } else {
                    if(words_found[result.second].first > loc) words_found[result.second] = {loc, j};
    ...
    REP(i, 0, c) {
        REP(j, 0, 8) {
            ...
            auto found = dict.search_Aho_Corasick(s);
            for(const auto &result : found) {
                // {0, i}에서 j방향으로 result.first-result.second.size()+1칸 이동한 위치
                int dist = result.first - result.second.size() + 1;
                pii loc = {dist * dx[j], i + dist * dy[j]};
                if(words_found.find(result.second) == words_found.end()) {
                    words_found[result.second] = {loc, j};
                } else {
                    if(words_found[result.second].first > loc) words_found[result.second] = {loc, j};
            ...
            found = dict.search_Aho_Corasick(s);
            for(const auto &result : found) {
                // {l-1, i}에서 j방향으로 result.first-result.second.size()+1칸 이동한 위치
                int dist = result.first - result.second.size() + 1;
                pii loc = {l - 1 + dist * dx[j], i + dist * dy[j]};
                if(words_found.find(result.second) == words_found.end()) {
                    words_found[result.second] = {loc, j};
                } else {
                    if(words_found[result.second].first > loc) words_found[result.second] = {loc, j};
    ...
    FOREACH(it, words) {
        cout << words_found[*it].first.first << ' ' << words_found[*it].first.second << ' ' << static_cast<char>(words_found[*it].second + 'A') << '\n';
    }
}