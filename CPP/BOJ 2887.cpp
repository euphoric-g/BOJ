...
int main() {
    ...
    int n;
    cin >> n;
    typedef tuple<int, int, int, int> data_type;
    vector<data_type> vec; // x, y, z, 행성번호
    UnionFind uf(n);
    REP(i, 0, n) {
        int x, y, z;
        cin >> x >> y >> z;
        vec.push_back(make_tuple(x, y, z, i));
    }
    auto dist = [](const data_type &a, const data_type &b) -> int {
        return min(min(abs(get<0>(a) - get<0>(b)), abs(get<1>(a) - get<1>(b))), abs(get<2>(a) - get<2>(b)));
    };
    auto comp_x = [](const data_type &a, const data_type &b) -> bool { return get<0>(a) < get<0>(b); };
    auto comp_y = [](const data_type &a, const data_type &b) -> bool { return get<1>(a) < get<1>(b); };
    auto comp_z = [](const data_type &a, const data_type &b) -> bool { return get<2>(a) < get<2>(b); };
    ll cost = 0;
    vector<piii> edges;
    sort(ALL(vec), comp_x);
    REP(i, 0, SIZE(vec)-1) {
        edges.push_back({{get<3>(vec[i]), get<3>(vec[i+1])}, dist(vec[i], vec[i+1])});
    }
    sort(ALL(vec), comp_y);
    REP(i, 0, SIZE(vec)-1) {
        edges.push_back({{get<3>(vec[i]), get<3>(vec[i+1])}, dist(vec[i], vec[i+1])});
    ...
    sort(ALL(edges), [](const piii &a, const piii &b) -> bool {
        return a.second < b.second;
    });
    REP(i, 0, SIZE(edges)) {
        if(uf.find_root(edges[i].first.first) == uf.find_root(edges[i].first.second)) continue;
        cost += edges[i].second;
        uf.merge(edges[i].first.first, edges[i].first.second);
    }
    cout << cost << '\n';
}