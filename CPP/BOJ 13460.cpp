...
pos move_sub(pos &cur, int &dir, char color) {
    ...
    if(color == RED) {
        ...
        return std::make_tuple(rx, ry, bx, by);
    } else {
        while(1) {
            // std::cout << rx << ", " << ry << ", " << bx << ", " << by << '\n';
            if(bx == hx && by == hy) return std::make_tuple(rx, ry, bx, by);
...
int main() {
    ...
    auto tie = [](int &rx, int &ry, int &bx, int &by, pos tp) { std::tie(rx, ry, bx, by) = tp; };
    int time = -1;
    std::queue<pos> q;
    visited[a][b][c][d] = true;
    q.push(std::make_tuple(a, b, c, d));
    while(!q.empty()) {
        int sz = q.size();
        if(++time > 10) break;
        while(sz-->0) {
