...
// Global Variables
regex slump_end("[D|E]F+G");
regex slump_recursive("[D|E]F+");
regex slimp("A[(.+)C]");

// functions
bool is_slump(string s) {
    // cout << "is_slump(" << s << ")\n";
    if(regex_match(s, slump_end)) return true;
    smatch match;
    if(regex_search(s, match, slump_recursive)) {
        // cout << match.suffix() << '\n';
        return is_slump(match.suffix());
    }
    // cout << match.suffix() << '\n';
    return false;
}
...
bool is_slimp(string s) {
    ...
    smatch match;
    if(s[0] == 'A' && s.back() == 'C') {
        string subs = SUBSTR(s, 1, s.size()-2);
        if(subs.size() < 3) return false;
        if(subs[0] == 'B') {
            return is_slimp(SUBSTR(subs, 1));
        } else {
            return is_slump(subs);
        }
    } else return false;
}
...
int main() {
    ...
    while(n--) {
        ...
        while(loc > 0 && pattern[loc] == 'D' || pattern[loc] == 'E' || pattern[loc] == 'F' || pattern[loc] == 'G') --loc;
        string first = SUBSTR(pattern, 0, loc+1), second = loc+1 < pattern.size() ? SUBSTR(pattern, loc+1) : "";
        if(first.size() < 2 || second.size() < 3) {
            cout << "NO\n";
            continue;
        }
        bool b1 = is_slimp(first), b2 = is_slump(second);
        if(b1 && b2) {
            cout << "YES\n";
        } else cout << "NO\n";
    }
