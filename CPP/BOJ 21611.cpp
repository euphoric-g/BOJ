...
// typedefs
typedef tuple<int, int, int> dat; // x, y, biz_num
typedef vector<dat> vdat;

// Global Variables
const int LEFT = 0, DOWN = 1, RIGHT = 2, UP = 3;
const int SUP = 1, SDOWN = 2, SLEFT = 3, SRIGHT = 4;
const int dx[] = {0, 1, 0, -1}, dy[] = {-1, 0, 1, 0};
const int sdx[] = {-1, 1, 0, 0}, sdy[] = {0, 0, -1, 1};

int n, m, score = 0;

// functions
vdat field_to_vdat(vvint &field) {
    int x = n/2, y = n/2, dir = LEFT;
    vdat ret;
    REP(i, 2, 2*n+1) {
        REP(j, 0, (i == 2*n ? i/2-1 : i/2)) {
            x += dx[dir], y += dy[dir];
            ret.push_back(make_tuple(x, y, field[x][y]));
        }
        dir = (dir + 1) % 4;
    }
    return ret;
}
// field 정보로부터 {x, y, 구슬 번호}를 순서대로 vector에 담아 반환

bool explode(vdat &vec) {
    // 4개 이상 연속된 구슬 탐지하여 제거
    // 한 개 이상 제거하면 true 반환
    stack<dat> st;
    bool ret = false;
    int type = 0, cnt = 0;
...
vdat change(vdat from) {
    vdat to;
    int type = 0, cnt = 0;
    REP(i, 0, SIZE(from)) {
        int num = get<2>(from[i]);
        if(type == num) {
            ++cnt;
        } else {
            if(type != 0) {
                to.push_back(make_tuple(-1, -1, cnt));
                to.push_back(make_tuple(-1, -1, type));
            }
            type = num;
            cnt = 1;
        }
...
void cast(vvint &field, pii spell) {
    int x = n/2, y = n/2, sdir = spell.first-1;
    REP(i, 1, spell.second+1) {
        int nx = x + i*sdx[sdir], ny = y + i*sdy[sdir];
        field[nx][ny] = 0;
    }
}

void vdat_to_field(vvint &field, vdat vec) {
    REP(i, 0, n) REP(j, 0, n) field[i][j] = 0;
    REV(vec);
    int x = n/2, y = n/2, dir = LEFT;
    REP(i, 2, 2*n+1) {
        REP(j, 0, (i == 2*n ? i/2-1 : i/2)) {
            x += dx[dir], y += dy[dir];
            while(!vec.empty() && get<2>(vec.back()) == 0) vec.pop_back();
            if(!vec.empty()) {
                field[x][y] = get<2>(vec.back()); vec.pop_back();
            } else field[x][y] = 0;
        }
        dir = (dir + 1) % 4;
    }
...
int main() {
    FASTIO;
    cin >> n >> m;
    auto field = GET_2D(n, n);
    queue<pii> spell_cast;
    REP(i, 0, m) {
        int a, b;
        cin >> a >> b;
        spell_cast.push({a, b});
    }
    auto PRINT_VDAT = [](vdat &vd) -> void {
        FOREACH(it, vd) {
            int a, b, c;
            tie(a, b, c) = *it;
            printf("{%d,%d,%d} ", a, b, c);
    ...
    vector<dat> vec = field_to_vdat(field);
    while(!spell_cast.empty()) {
        cast(field, spell_cast.front()); spell_cast.pop();
        auto vd = field_to_vdat(field);
        vdat_to_field(field, vd);
        vd = field_to_vdat(field);
        // cast 후 빈칸 제거
        while(explode(vd)) {
            vdat_to_field(field, vd);
            vd = field_to_vdat(field);
