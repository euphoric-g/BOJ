...
int main() {
    ...
    cin >> n >> m;
    n *= 24;
    auto base = GET_N_INTS(m);
    auto add = GET_N_INTS(m);
    priority_queue<pii, vpii, pq_cmp::pii_110> pq;
    REP(i, 0, m) pq.push({base[i], min(100 - base[i], add[i])});
    int score_sum = 0;
    while(n > 0 && !pq.empty()) {
        auto get = pq.top(); pq.pop();
        // cout << get.first << ' ' << get.second << " -> ";
        if(get.second == 0 || get.first == 100) {
            score_sum += get.first;
            // cout << "score_sum = " << score_sum << '\n';
            continue;
        }
        int time_need = (100 - get.first) / get.second;
        time_need = min(n, time_need);
        n -= time_need;
        get.first += time_need * get.second;
        get.second = min(get.second, 100 - get.first);
        // cout << get.first << ' ' << get.second << '\n';
        pq.push(get);
    }
    while(!pq.empty()) {
        score_sum += pq.top().first;
        pq.pop();
    }
    cout << score_sum << '\n';
}